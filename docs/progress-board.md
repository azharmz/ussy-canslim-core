...
**COMPLETE. Main conclusion: CAN SLIM letters do not share one Boolean role.**

Frozen role taxonomy:
...
| S | **supply-demand confirmation/evidence** | float/shares/buyback descriptor | **PARTIAL** — breakout volume strong, full S incomplete |
| I | **institutional sponsorship confirmation/evidence** | sponsorship trend / sponsor quality | **PARTIAL** — manager-count delta narrow proxy |
...
Key decision: historical v1 ablations answer whether a **specific proxy** improved v1 performance; they do not redefine the theoretical importance of the underlying CAN SLIM component. Hard C/I or stricter M not improving historical CAGR/PF does not mean C/I/M are irrelevant to O'Neil methodology.

For #32, do not force all components into identical `PASS/FAIL`. Preserve screening, confirmation/evidence, context/timing and `NOT_EVALUABLE/NOT_IMPLEMENTED` states separately.
...
1. **Execute #31 Sell / Risk-Management Fidelity.**
2. Then freeze #32 Theory-Faithful Candidate Specification.
3. Only after #32, begin #33 Pattern Recognition Engine using R2 OHLCV.
4. Keep FWD1 and EXH2 accumulating unchanged.
5. Do not tune theory/pattern/breakout/RS/C/A/S/I/M rules from historical or interim forward performance.