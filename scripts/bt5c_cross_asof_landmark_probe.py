"""BT5C exact cross-asof raw-landmark reuse probe.
Frozen O'Neil is imported unchanged. Later-prefix raw landmarks are filtered by
confirmed_date; fusion is deliberately recomputed on each earlier causal frame
because frozen fusion contains frame-relative boundary/separation evidence.
"""
...
CONTRACT="BT5C_CROSS_ASOF_RAW_LANDMARK_EQUIVALENCE_PROBE_V2"
...
def extract_all(frame):
 t=time.perf_counter(); exc=extract_excursion_landmarks(frame); a=time.perf_counter(); win=extract_confirmed_window_landmarks(frame); b=time.perf_counter(); fused=fuse_landmark_sources(frame,exc,win); c=time.perf_counter()
 return {"excursion_objects":exc,"window_objects":win,"excursion":canonical(exc),"window":canonical(win),"fused":canonical(fused),"timing_sec":{"excursion":round(a-t,6),"window":round(b-a,6),"fusion":round(c-b,6),"total":round(c-t,6)}}
...
def run_probe(frame,asof_dates):
 dates=sorted(set(asof_dates)); latest=dates[-1]; lf=prefix(frame,latest); ls=extract_all(lf); cases=[]; all_exact=True
 for asof in dates[:-1]:
  earlier=prefix(frame,asof); oracle=extract_all(earlier)
  ...
  t=time.perf_counter(); reused_fused_obj=fuse_landmark_sources(earlier,reused_exc_obj,reused_win_obj); fusion_reuse_sec=round(time.perf_counter()-t,6)
  components={"excursion":cmp(oracle["excursion"],canonical(reused_exc_obj)),"window":cmp(oracle["window"],canonical(reused_win_obj)),"fused_recomputed":cmp(oracle["fused"],canonical(reused_fused_obj))}
  exact=all(x["exact"] for x in components.values()); all_exact=all_exact and exact
  cases.append({"asof_date":asof.isoformat(),"bars":len(earlier),"oracle_timing_sec":oracle["timing_sec"],"reuse_fusion_timing_sec":fusion_reuse_sec,"components":components,"exact":exact})
 return {"contract":CONTRACT,"frozen_oneil_sha":FROZEN_ONEIL_SHA,"latest_asof_date":latest.isoformat(),"latest_bars":len(lf),"latest_extraction_timing_sec":ls["timing_sec"],"cases":cases,"all_exact":all_exact,"landmark_cross_asof_reuse_authorized":False,"morphology_cross_asof_reuse_authorized":False,"bounded_replay_authorized":False,"production_eligibility_emitted":False,"strategy_returns_computed":False,"verdict":"RAW_LANDMARK_REUSE_EQUIVALENCE_OBSERVED_MORE_CASES_REQUIRED" if all_exact else "RAW_LANDMARK_REUSE_NOT_EXACT_FAIL_CLOSED"}
