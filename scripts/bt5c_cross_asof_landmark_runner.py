...
def main():
 ...
 for i,key in enumerate(discovery_keys,1):
  ...
  if len(selected)>=target:
   log(f"deterministic corpus target reached after {i} discoveries; stopping discovery early")
   break
 if len(selected)<target:
  payload={"contract":"BT5C_FINAL_OUTPUT_LOCAL_TURN_REUSE_EQUIVALENCE_V1","frozen_oneil_sha":ONEIL_SHA,"upstream_history_contract":UPSTREAM_CONTRACT,"history_prefix":prefix,"inventory_count":len(inventory),"selection":{"method":"deterministic_even_spread_expanded_until_target_or_cap_v2","discovery_cap":discovery,"target_sample":target,"max_complete_history_bars":max_bars,"selected_keys":selected,"discovery":discovery_rows},"all_exact":False,"final_production_output_tested":False,"global_reuse_authorized":False,"verdict":"INCONCLUSIVE_INSUFFICIENT_DETERMINISTIC_CORPUS"}; write(payload); raise RuntimeError(f"insufficient deterministic final-output corpus: need={target} got={len(selected)} cap={discovery}")
