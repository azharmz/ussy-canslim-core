...
def client(): return boto3.client("s3",endpoint_url=env("R2_ENDPOINT"),aws_access_key_id=env("R2_ACCESS_KEY_ID"),aws_secret_access_key=env("R2_SECRET_ACCESS_KEY"),region_name="auto")
...
def compress_one(s3,bucket,key):
    if key.endswith(".gz"):
        head=s3.head_object(Bucket=bucket,Key=key); return {"key":key,"stored_size_bytes":head["ContentLength"],"already_compressed":True}
    gz_key=key+".gz"
    with tempfile.TemporaryDirectory() as td:
        src=os.path.join(td,"source.jsonl"); dst=os.path.join(td,"source.jsonl.gz")
        s3.download_file(bucket,key,src); logical_sha=file_sha(src); logical_size=os.path.getsize(src)
        with open(src,"rb") as fin, open(dst,"wb") as raw_out:
            with gzip.GzipFile(filename="",mode="wb",fileobj=raw_out,compresslevel=6,mtime=0) as fout: shutil.copyfileobj(fin,fout,length=8*1024*1024)
        ...
        if s3.head_object(Bucket=bucket,Key=gz_key)["ContentLength"]!=stored_size: raise RuntimeError(f"uploaded size mismatch: {gz_key}")
...
def main():
    ...
    old_candidate=ptr["candidates_key"]; old_pattern=manifest.get("artifacts",{}).get("patterns.jsonl",{}).get("key")
    ...
    artifacts=manifest.setdefault("artifacts",{}); artifacts.pop("candidates.jsonl",None); artifacts["candidates.jsonl.gz"]={k:v for k,v in candidate.items() if k not in ("source_key","already_compressed")}
    if pattern:
    ...
    mbytes=json.dumps(manifest,indent=2,sort_keys=True,default=str).encode(); mkey=ptr["manifest_key"]; s3.put_object(Bucket=bucket,Key=mkey,Body=mbytes,ContentType="application/json")
