...
def main():
 ...
 for sym,(pivot,asof) in CASES.items():
  print(f"R1-E case start: {sym}",flush=True)
  try:
   routed=route_development_ohlcv(sym,asof-timedelta(days=650),asof+timedelta(days=2));f=routed.frame
   source=routed.source.value if hasattr(routed.source,"value") else str(routed.source)
   fallback=routed.fallback_reason
   print(f"R1-E source: {sym}: {source}; fallback={fallback}",flush=True)
  except Exception as exc:
   out.append({"symbol":sym,"oracle_pivot":pivot,"result":"DATA_UNAVAILABLE","error":str(exc)})
   print(f"R1-E data unavailable: {sym}: {exc}",flush=True);continue
  f["date"]=pd.to_datetime(f["date"]);d=f[f.date.dt.date<=asof].copy();obs=[x.to_dict() if hasattr(x,"to_dict") else x.__dict__ for x in analyze_security(sym,sym,d,asof)];fb=[x for x in obs if x.get("pattern")=="FLAT_BASE"]
  provenance={"ohlcv_source":source,"ohlcv_fallback_reason":fallback}
  if not fb:out.append({"symbol":sym,"oracle_pivot":pivot,**provenance,"result":"NO_FLAT_BASE_CANDIDATE"});continue
  fb.sort(key=lambda x:abs(float(x["pivot_level"])-pivot));p=float(fb[0]["pivot_level"]);lin=[x for x in fb if abs(float(x["pivot_level"])-p)<1e-6];out.append({"symbol":sym,"oracle_pivot":pivot,**provenance,"nearest_delta_pct":p/pivot-1,"lineage":[{"semantics":x["candidate_semantics"],"state":x["normalized_status"],"start":x["structural_start"],"end":x["structural_end"],"pivot":x["pivot_level"],"faults":x["detector_faults"],**metrics(d,x)} for x in lin]})
