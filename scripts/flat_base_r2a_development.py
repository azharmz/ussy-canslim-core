...
CASES={
 "MSFT":(430.82,date(2024,6,4)),
 "BRK-B":(430.00,date(2024,6,4)),
 "ANET":(292.66,date(2024,3,15)),
 "ARES":(139.48,date(2024,3,15)),
 "HUBS":(660.00,date(2024,3,15)),
 "ALL":(168.05,date(2024,3,5)),
 "TDG":(542.20,date(2019,10,29)),
}

def metrics(f,r):
 import pandas as pd
 s=pd.Timestamp(r["structural_start"]).date(); e=pd.Timestamp(r["structural_end"]).date()
 z=f[(f.date.dt.date>=s)&(f.date.dt.date<=e)].copy()
 ...
 chg=w.close.pct_change().dropna(); sgn=[1 if x>0 else -1 if x<0 else 0 for x in chg]
 sc=sum(a and b and a!=b for a,b in zip(sgn[:-1],sgn[1:]))
 wr=(w.high-w.low)/w.high; prior=wr.iloc[:-2]
 return {
  "duration_sessions":len(z),"trading_weeks":len(w),"depth_pct":r["depth_pct"],
  "normalized_range":nr,"close_dispersion":cd,
  "frozen_tight":nr<=.03 and cd<=.01,"frozen_wide_loose":nr>=.07 or cd>=.03,
  ...
  "weekly_direction_changes":int(sc),
...
def main():
 ...
 for sym,(pivot,asof) in CASES.items():
  print(f"R2-A case start: {sym}",flush=True)
  try:
   routed=route_development_ohlcv(sym,asof-timedelta(days=650),asof+timedelta(days=2))
