...
SYMBOL = "FTCH"
ASOF = date(2020,10,23)
EXECUTION_END = date(2020,10,26)
START = date(2019,10,1)
ORACLE = {"pattern":"CUP_WITH_HANDLE","pivot":28.91,"breakout_date":"2020-10-23","source":"IBD Top Stocks 2020"}
# Provider historical basis retained unchanged; source price neighborhood is checked against the 28.91 oracle pivot.
ROOT=Path("artifacts/historical-reconstruction/ftch-2020")
...
def main():
    ...
    if frame is None or frame.empty:
        ...
        if tiingo_key:
            url=("https://api.tiingo.com/tiingo/daily/"+SYMBOL+"/prices?"+urllib.parse.urlencode({"startDate":START.isoformat(),"endDate":EXECUTION_END.isoformat(),"token":tiingo_key}))
            try:
                with urllib.request.urlopen(url,timeout=30) as resp: rows=json.load(resp)
                if rows:
                    frame=pd.DataFrame(rows).rename(columns={"adjClose":"adj_close"})
        ...
        if frame is None or frame.empty:
            ...
            if twelve_key:
                ...
                try:
                    with urllib.request.urlopen("https://api.twelvedata.com/time_series?"+params,timeout=30) as resp: payload=json.load(resp)
                    ...
                    if vals:
                        ...
                        frame=frame.sort_values("date").reset_index(drop=True); frame["adj_close"]=frame["close"]; provider="Twelve Data"
                except Exception as exc:
    ...
    csv=ROOT/"ohlcv-2020-provider-basis.csv"
    contemporaneous.to_csv(csv,index=False)
    sha=hashlib.sha256(csv.read_bytes()).hexdigest()
    observed=analyze_security("FTCH",SYMBOL,detector_frame,ASOF)
