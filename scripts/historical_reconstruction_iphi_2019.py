...
SPLIT_FACTOR = 1.0
ROOT=Path("artifacts/historical-reconstruction/iphi-2019")
...
def main():
    ...
    for index_id,ticker in market_ids.items():
        market_raw[index_id]=fetch_yfinance(ticker,date(2019,5,1),ASOF)
    ...
    # IPHI replay uses the provider historical basis unchanged.
    ...
    observed=analyze_security("IPHI",SYMBOL,detector_frame,ASOF)
    ...
    target=[]  # IPHI oracle landmarks are not injected into detector selection; inspect emitted candidates first.
    target_diag=None
    if target:
        seg=target[0]
