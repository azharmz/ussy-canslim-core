...
SYMBOL = "NVO"
ASOF = date(2024,6,4)
EXECUTION_END = date(2024,6,5)
START = date(2023,6,1)
ORACLE = {"pattern":"CUP_WITH_HANDLE","pivot":137.22,"breakout_date":"2024-06-04","source":"IBD / MarketSurge"}
# Yahoo historical bars are normalized to the contemporaneous 2019 pre-split basis below.
ROOT=Path("artifacts/historical-reconstruction/nvo-2024")
...
def main():
    ...
    observed=analyze_security("NVO",SYMBOL,detector_frame,ASOF)
    ...
    payload={
      ...
      "source":{"provider":"Yahoo via yfinance","auto_adjust":False,"start":START.isoformat(),"end":EXECUTION_END.isoformat(),"detector_asof":ASOF.isoformat(),"rows":len(frame),"yahoo_split_adjusted_sha256":raw_sha},
        "price_basis":{"classification":"DATA_ADAPTATION","basis":"provider basis retained pending source-neighborhood sanity check","price_transform":"none","volume_transform":"none","provider_basis_sha256":sha},
