...
SYMBOL = "TSM"
ASOF = date(2024,5,10)
EXECUTION_END = date(2024,5,13)
LIFECYCLE_END = date(2024,8,31)
START = date(2023,7,1)
ORACLE = {"pattern":"DOUBLE_BOTTOM","pivot":148.43,"breakout_date":"2024-05-10","source":"IBD / MarketSurge"}
SPLIT_FACTOR = 1.0
ROOT=Path("artifacts/historical-reconstruction/tsm-2024")
...
def main():
    ...
    if frame is None or frame.empty:
        raise RuntimeError("TSM historical OHLCV unavailable from Yahoo, Tiingo, and Twelve Data with configured credentials")
    ...
    for index_id,ticker in market_ids.items():
        market_raw[index_id]=fetch_yfinance(ticker,date(2023,11,1),ASOF)
    ...
    # TSM replay uses the provider historical basis unchanged.
    ...
    observed=analyze_security("TSM",SYMBOL,detector_frame,ASOF)
    ...
    target=[]  # TSM oracle landmarks are not injected into detector selection; inspect emitted candidates first.
    target_diag=None
    if target:
        seg=target[0]
