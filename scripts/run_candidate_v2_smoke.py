...
def main() -> None:
    ...
    # Canonical #33 production execution; parent does not reimplement morphology.
    ...
    for p in pattern_rows:
        ...
        output.append(row)
        adapter_reasons[p.assessment_id] = list(adapted.reason_codes)
    ...
    summary = {
        ...
        "candidate_stage_counts": {str(k): int(v) for k, v in counts.items()},
        "pattern_status_counts": {str(k): int(v) for k, v in status_counts.items()},
        "advanced_patterns_allowed": False,
        "trading_performance_metrics_used": False,
        "m_proxy": "SPY_ONLY_TRANSPARENT_PROXY",
        "i_state": "NOT_EVALUABLE_UNLESS_SEPARATE_PIT_SOURCE_ATTACHED",
