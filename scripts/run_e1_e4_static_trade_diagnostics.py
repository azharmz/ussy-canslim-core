...
def simulate_symbol(
    ...
    for row in candidates.itertuples(index=False):
        ...
        base = {
            ...
            "tminus1_to_t0": row.tminus1_to_t0,
            "h1_open": row.h1_open,
            "h1_gap": row.h1_gap,
            "t0_pivot_extension": row.t0_pivot_extension,
            "h1_fill_extension": row.h1_fill_extension,
            "rs_percentile": row.rs_percentile,
            "volume_ratio": row.volume_ratio,
        }
        if idx is None or idx + 1 >= len(history):
            events.append({**base, "entry_event": "NO_H1_BAR"})
            continue
        if idx + 1 <= active_until_idx:
            events.append({**base, "entry_event": "SKIP_ALREADY_OPEN"})
            continue
        ...
        stop = float(dec.hard_stop)
        target = float(dec.profit_target)
        pre_exit_max_high = entry
        pre_exit_min_low = entry
        ...
        for j in range(entry_idx, len(history)):
            ...
            if result.exited:
                ...
                break
            pre_exit_max_high = max(pre_exit_max_high, float(bar["high"]))
            pre_exit_min_low = min(pre_exit_min_low, float(bar["low"]))

        if exit_idx is None:
            exit_idx = len(history) - 1
            exit_price = float(history.iloc[exit_idx]["close"])
            realized_return = None
            mark_return = exit_price / entry - 1.0
        else:
            realized_return = exit_price / entry - 1.0
            mark_return = realized_return
        ...
        trades.append({
            **base,
            ...
            "holding_sessions": int(exit_idx - entry_idx + 1),
            "realized_return": realized_return,
            "mark_return": mark_return,
            "pre_exit_mfe": pre_exit_max_high / entry - 1.0,
            "pre_exit_mae": pre_exit_min_low / entry - 1.0,
        })
...
def descriptive_by_quantile(df: pd.DataFrame, feature: str) -> list[dict]:
    ...
    if len(x) < 4:
        return []
    x["bucket"] = pd.qcut(x[feature], 4, duplicates="drop")
    rows: list[dict] = []
    for bucket, g in x.groupby("bucket", observed=True):
        ...
        rows.append({
            ...
            "target_rate": float((g["exit_reason"].str.contains("TARGET")).mean()),
            "stop_rate": float((g["exit_reason"].str.contains("STOP")).mean()),
            "censored_rate": float((g["exit_reason"] == "CENSORED_OPEN").mean()),
            "median_realized_return": float(realized.median()) if len(realized) else None,
            "median_pre_exit_mfe": float(g["pre_exit_mfe"].median()),
            "median_pre_exit_mae": float(g["pre_exit_mae"].median()),
        })
    return rows
...
def extension_threshold_table(events: pd.DataFrame) -> list[dict]:
    ...
    for threshold in (0.03, 0.05, 0.08):
        valid = pd.to_numeric(events["h1_fill_extension"], errors="coerce")
        rows.append({
            "threshold": threshold,
            "candidate_events_with_h1": int(valid.notna().sum()),
            "within_threshold": int((valid <= threshold).sum()),
            "above_threshold": int((valid > threshold).sum()),
        })
    return rows
...
def main() -> None:
    ...
    failures: list[dict] = []
    grouped = {str(sid): g.copy() for sid, g in candidates.groupby("security_id")}

    with ThreadPoolExecutor(max_workers=16) as pool:
        futures = {pool.submit(load_full_history, s3, bucket, sid): sid for sid in grouped}
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                failures.append({"security_id": sid, "error": str(exc)[:300]})
    if failures:
        raise RuntimeError(f"History load failures: {failures[:5]}")

    for sid, g in grouped.items():
        trades, events = simulate_symbol(sid, g, histories[sid])
    ...
    events.to_csv(OUT / "entry_events.csv", index=False)

    quantile_rows: list[dict] = []
    for feature in ("tminus1_to_t0", "h1_gap", "t0_pivot_extension", "h1_fill_extension"):
        quantile_rows.extend(descriptive_by_quantile(trades, feature))
    pd.DataFrame(quantile_rows).to_csv(OUT / "quantile_diagnostics.csv", index=False)
    pd.DataFrame(extension_threshold_table(events)).to_csv(OUT / "fill_extension_3_5_8.csv", index=False)
    ...
    # PF is reported only as a descriptive trade-level diagnostic on biased static-universe evidence.
    pf = gross_profit / gross_loss if gross_loss > 0 else None

    event_counts = {str(k): int(v) for k, v in events["entry_event"].value_counts().to_dict().items()} if len(events) else {}
    exit_counts = {str(k): int(v) for k, v in trades["exit_reason"].value_counts().to_dict().items()} if len(trades) else {}
    summary = {
        "experiment": "E1-E4-STATIC-TRADE-DIAGNOSTICS-V1",
        ...
        "censored_open_count": int((trades["exit_reason"] == "CENSORED_OPEN").sum()) if len(trades) else 0,
        "descriptive_trade_level_pf": pf,
        "median_realized_return": float(realized.median()) if len(realized) else None,
        ...
        "feature_quantiles_all_candidates": {
            "tminus1_to_t0": quantiles(events["tminus1_to_t0"]),
            "h1_gap": quantiles(events["h1_gap"]),
            "t0_pivot_extension": quantiles(events["t0_pivot_extension"]),
            "h1_fill_extension": quantiles(events["h1_fill_extension"]),
        },
        "pre_specified_fill_extension_counts": extension_threshold_table(events),
        "market_proxy_scope": "SPY_ONLY",
        "execution_baseline": "X1_7PCT_STOP_20PCT_PIVOT_TARGET",
        "portfolio_metrics_computed": False,
        "allowed_use": "E1-E4 exploratory mechanism diagnostics only. Do not tune thresholds or infer unbiased strategy performance.",
        "survivorship_warning": "2026-08-28 membership is held fixed across historical dates; historical eligibility is not PIT.",
