...
def main() -> None:
    ...
    match_cusips = set(matched["derived_cusip9"])
    matched_info = stock_rows[stock_rows["CUSIP"].isin(match_cusips)].copy()
    ...
    summary = {
        ...
        "us_isin_deterministically_mappable_count": int(universe["us_isin_mappable"].sum()),
        "non_us_isin_deferred_count": int((~universe["us_isin_mappable"]).sum()),
        "us_isin_mapping_fraction": float(universe["us_isin_mappable"].mean()),
        ...
        "bulk_unique_stock_cusips": int(stock_rows["CUSIP"].loc[stock_rows["CUSIP"].str.len().eq(9)].nunique()),
