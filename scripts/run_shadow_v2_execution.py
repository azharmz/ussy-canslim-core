...
def main():
    ...
    rb=raw(s3,bucket,c["ready_key"])
    if hashlib.sha256(rb).hexdigest()!=c["ready_sha256"]: raise RuntimeError("READY_SHA256_MISMATCH")
    ...
    decisions=[]
    for r in eligible:
        sid=str(r["security_id"]); g=ready[ready["security_id"].astype(str).eq(sid)].sort_values("date")
    ...
    out={"schema_version":1,"type":"canslim_v2_shadow_execution","decision_date":c["decision_date"],
         ...
         "watchlist_sha256":c["watchlist_sha256"],"ready_key":c["ready_key"],"ready_sha256":c["ready_sha256"],
