...
from canslim_research.pattern_breakout_entry_v1 import (
    EntryState,
    TECHNICAL_ELIGIBLE_STAGE,
    decide_pattern_breakout_t1_open,
    validate_pattern_breakout_entry,
)
...
def decide(open_price: float):
    return decide_pattern_breakout_t1_open(
        ...
        pivot_level=100.0,
        next_session_date=date(2026, 9, 18),
...
def test_above_five_percent_is_missed_extended():
    x = decide(105.01)
    assert x.entry_state == EntryState.MISSED_EXTENDED_AT_OPEN
    assert x.fill_price is None


def test_below_pivot_does_not_fill():
    x = decide(99.99)
    assert x.entry_state == EntryState.BELOW_PIVOT_AT_OPEN
    assert x.fill_price is None


def test_no_canslim_stage_alias_is_accepted():
